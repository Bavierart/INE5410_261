...
void * thread(void * arg) {
	...
	for (int i = 0; i < args->operations; i++) {
        args->sum[i] = args->first[i] + args->second[i];
	}
...
int main(int argc, char* argv[]) {
    ...
    //pega o número de threads a ser criado
    int stop = n_threads > a_size ? a_size : n_threads;
    //numero de operações por thread (equivalente a ceil(size/stop) mas com int)
    ...
    for (int i = 0; i < stop; i++) {
        ...
        threadArg_t *args = malloc(sizeof(threadArg_t));
        args->first = &a[begin];
        args->second = &b[begin];
        args->sum = &c[begin];
        args->operations = end - begin;

	   pthread_create(&threads[i], NULL, thread, ((void*) (args))); 
    }
